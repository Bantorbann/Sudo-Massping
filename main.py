...
Sudo = discord.Client()
Sudo = commands.Bot(description="Sudo Selfbot", command_prefix="$", self_bot=True, intents=intents, case_insensitive=True, guild_subscriptions=True)

def Main():
    try:
        Sudo.run(token, bot=False)
...
@Sudo.command(aliases=["stop", "st", "shutdown"])
